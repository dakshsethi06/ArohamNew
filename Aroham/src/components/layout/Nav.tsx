...
export function Nav() {
  ...
  const solid = scrolled || isCartPage;
    ...
    <nav role="navigation" aria-label="Main navigation"
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 flex items-center justify-between h-16 lg:h-20">
        ...
        <div className="lg:hidden flex items-center gap-3" style={{ color: solid ? MAROON : IVORY }}>
          ...
          <button aria-expanded={open} aria-controls="mobile-menu" onClick={() => setOpen(!open)} className="p-1">{open ? <X size={22} /> : <Menu size={22} />}</button>
        </div>
      </div>
