...
export function HexPrismCarousel({ products, onProductClick }: { products: ArohamProduct[]; onProductClick: (p: ArohamProduct) => void }) {
  ...
  return (
    <div className="relative flex flex-col items-center w-full flex-1" style={{ transition: `all ${SMOOTH_EASE}` }}>
      ...
      <div style={{ position: isMobile ? "relative" : "absolute", bottom: isMobile ? undefined : "10px", marginTop: isMobile ? (expanded ? "-20px" : "0px") : undefined, left: "5%", right: "5%", width: "90%", zIndex: 50, overflow: "hidden", maxHeight: expanded ? 220 : 0, opacity: expanded ? 1 : 0, transition: `all ${SMOOTH_EASE}`, pointerEvents: expanded ? "auto" : "none" }}>
